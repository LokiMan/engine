...
describe 'Repository', ->
  ...
  describe 'has', ->
    it 'should call storage.has', ->
      storage = {has: spy()}
      repository = createRepository {collection: 'data', storage}

      repository.has 'uid1'

      expect(storage.has.calls).to.not.empty

    it 'should not call storage.has if has in cache', ->
      storage = {has: spy()}
      repository = createRepository {collection: 'data', storage}
      repository.add 'uid1', {}
      ...
      expect(storage.has.calls).to.have.lengthOf 1