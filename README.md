...
- в папке examples есть несколько примеров игр-демонстраций работы с движком:
...
- создание игры начинается с файла, в котором задаются сцены и их компоненты (может находиться в `src/game.coffee`, или `src/main.coffee`)
- сцена, в которой начинает новый игрок, по умолчанию называется `'start'` (можно изменить с помощью параметра `startScene` в файле игры, как сделано в примере `quest`)
